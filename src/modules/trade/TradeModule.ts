...
// Market state (shared across all Trade instances)
let marketMults = new Map<string, number>();

function initMarket(): void {
  if (marketMults.size === 0) {
...
function renderTradeList(container: HTMLElement): void {
  const tradeList = container;
  tradeList.innerHTML = '';
...
/**
 * Mount Trade as a panel (original behavior, for backward compatibility)
 */
...
/**
 * Open Trade as a modal overlay.
 * Returns a function to close the modal.
 */
export function showTradeModal(): () => void {
  initMarket();
  ...
  modal.innerHTML = `
    <div class="modal__content">
      <div class="modal__header">
        <h2 class="modal__title">💱 Trade</h2>
  ...
  const tradeList = modal.querySelector<HTMLElement>('.trade-list')!;
